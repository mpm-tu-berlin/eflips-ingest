...
def add_or_ret_vehicle_type(scenario_id: int, fahrzeugtyp: str, session: Session) -> eflips.model.VehicleType:
    ...
    if vehicle_type is None:
        # Geometry: BVG-XML does not carry vehicle dimensions. Set a standard 12 m solo
        # city bus shape so the row satisfies downstream NOT NULL constraints (e.g.
        # django-simba) and eflips-model's all-three-set check constraint. The user is
        # expected to override these on the auto-generated row.
    ...
    if database_url.startswith("postgis") or database_url.startswith("postgres"):
        ...
        for table_name in TABLES:
            with conn.cursor() as cur:
                # Discover the actual sequence rather than assuming the SQLAlchemy
                # naming convention. Django creates sequences like
                # "<app>_<model>_id_seq" even when the table is named "Scenario".
                # pg_get_serial_sequence parses its first arg as an SQL identifier,
                # so PascalCase names must be passed double-quoted.
                cur.execute("SELECT pg_get_serial_sequence(%s, %s)", (f'"{table_name}"', KEY_NAME))
                row = cur.fetchone()
                sequence = row[0] if row is not None else None
                if sequence is None:
                    continue
